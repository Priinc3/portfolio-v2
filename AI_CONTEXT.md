# AI Context — Portfolio V2

## Overview
- **Purpose**: Personal portfolio for Priince Gondaliya, AI Automation Developer.
- **Stack**: HTML5, CSS3 (Vanilla), JavaScript (Vanilla).
- **Status**: Production
- **Version**: 1.0.0
- **Last Updated**: 2025-05-14

## File Structure
- `portfolio.html`: Main entry point and only page.
- `AI_CONTEXT.md`: This file.
- `CHANGELOG.md`: Project history.

## Key Components
| Component | Purpose |
|-----------|---------|
| Hero | Introduction with SIH badge and status. |
| About | Biography and key stats. |
| Experience | Career timeline. |
| Projects | Grid of projects with tech stacks. |
| Skills | Categorized technical skills. |
| Contact | Call to action and social links. |

## Typography (Updated v0.2.0)
- **Primary Display/Body**: 'Plus Jakarta Sans'
- **Mono/Technical**: 'JetBrains Mono'
- **Accents**: 'Bricolage Grotesque'

## Known Issues
- Custom cursor may feel sluggish on low-end devices.

## Next Steps
- [ ] Add more projects.
- [ ] Implement dark/light mode toggle (requested white theme default).
